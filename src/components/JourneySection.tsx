...
import { ICONS, MODULE_STATUS, LAUNCHED } from '@/data/modules';
...
const SCENES = [
  ...
  { id:'schrijft', name:'Schrijft', price:19,
  ...
  { id:'promoot', name:'Promoot', price:19,
...
const VIGNETTES: Record<string, string> = {
  boekt:`<div class="vig__row vig__row--head"><span class="vig__dot"></span> Slim inboeken</div>
...
function statusLabel(id: string): string {
  ...
  if (st.live) return 'nu live';
  if (id === 'boekt') return LAUNCHED ? 'nu live' : 'lanceert in juli';
  return st.when;
}
...
export default function JourneySection() {
  ...
  return (
    <section className="journey" id="modules" ref={journeyRef}>
      <div className="journey__sticky">
        <div className="journey__head">
          <div className="wrap">
            <div className="eyebrow"><span className="tick"></span> Acht modules, één Emma</div>
          </div>
        </div>
            ...
            <article className="scene" key={s.id} style={{ ['--sc' as string]: `var(--m-${s.id})` }}>
              <div className="scene__inner">
                <div className="scene__text">
                  <div className="scene__num">
                    <span className="ln"></span>Emma{s.name} · vanaf €{s.price}/mnd
                    <span className={`scene__status${statusLabel(s.id) === 'nu live' ? ' is-live' : ''}`}>{statusLabel(s.id)}</span>
                  </div>
                ...
                {PAIR.map(s => (
                  <Link className="pair__card" key={s.id} href={`/modules/${s.id}`} style={{ ['--sc' as string]: `var(--m-${s.id})` }}>
                    ...
                    <div className="pair__foot">
                      ...
                      <span className="pair__go">{MODULE_STATUS[s.id].when} · Bekijk <span className="arr">→</span></span>
                    </div>
                  </Link>
                ))}
              </div>
